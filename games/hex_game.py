...
class HexGame(Game):
    def __init__(self, n=7):
        ...
        self.action_mode = 'place'  # 告诉界面这是“落子”型游戏
        self.geometry = 'hex'      # 告诉界面使用六边形倾斜布局
    ...
    def action_from_canonical(self, action, player):
        ...
        if player == 1:
            ...
            # 由于 canonical 做了转置 (.T)，坐标 (r, c) 变成了 (c, r)
            # 我们需要把动作索引也转置回来
            r, c = divmod(action, self.n)
            return c * self.n + r

    def action_to_canonical(self, action, player):
        """
        将原始动作转换为 AI 视角的动作 (目前主要用于训练数据记录)
        """
        if player == 1:
            return action
        else:
